...
class SuffixTree(object):
	...
	def node_substring(self, numNode):
        word = ''
...
def longestRepeat(text):
	...
	nodeList = filter(lambda i: len(suffxTree.nodes[i].children) >= 2, range(len(suffxTree.nodes)))
	maxNodeDepth = 0
	maxIndex = 0
	for i in range(len(nodeList)):
		if maxNodeDepth < nodeList.nodeDepth(i):
			...
			maxIndex = i
	bestNode = nodeList[i]
	return suffxTree.nodeSubstring(bestNode)
